...
def record(md, verdict, round_no=None, source=None):
    ...
    with open(jp, "a+", encoding="utf-8") as journal:
        ...
        # Старый формат без подписи и историческая пометка «без Кони» сборку не
        # открывают и не занимают право на первую настоящую рецензию тех же байтов.
        # Любой ранее подписанный настоящий вердикт сохраняет прежний антидубль.
        signed_same = [
            e for e in same_sha
            if e.get("_verified")
            and not str(e.get("verdict", "")).casefold().startswith(
                NO_REVIEW_PREFIX.casefold())
        ]
        if signed_same:
            e = signed_same[-1]
