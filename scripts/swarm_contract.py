...
# ponytail: словоформы без морфоанализатора; если словарь разрастется,
# следующий потолок — штатный токенизатор перед заменой правила.
_LEGAL_STRONG = (
    r"\b(?:ст\.\s*|стать(?:я|и|е|ю|ей|ею|ям|ями|ях)\s+)\d+\b|"
    r"\b(?:ГК|ГПК|АПК|УК|КАС|ВС)\s+РФ\b|"
    r"\bпленум(?:а|у|ом|е|ы|ов|ам|ами|ах)?\b|"
    r"\bподсудност(?:ь|и|ью|ей|ям|ями|ях)\b|"
    r"\bисков(?:ая|ой|ую|ою)\s+давност(?:ь|и|ью)\b|"
    r"(?:(?:\bN|№)|\bдел(?:о|у)\s+(?:N|№)?)\s*"
    r"[А-ЯA-Z]\d{1,3}-\d+/\d{4}\b|"
    r"\bнорм(?:а|ы|е|у|ой|ою|ам|ами|ах)\s+"
    r"(?:материальн|процессуальн)(?:ого|ому|ым|ом|ое|ые|ых|ыми)\s+"
    r"прав(?:а|о|у|ом|е)\b|"
    r"\bобеспечени(?:е|я|ю|ем|и|й|ям|ями|ях)\s+"
    r"иск(?:а|у|ом|е|и|ов|ам|ами|ах)\b|"
    # Прежние сильные случаи остаются: LIM-01 запрещает ослаблять ворота.
    r"\bпостановлени(?:е|я|ю|ем|и|й|ям|ями|ях)\b|"
    r"\bпостановлен(?:а|о|ы)?\s+(?:взыскать|отказать|прекратить|запрет)\b|"
    r"\bподсудим(?:ый|ого|ому|ым|ом|ая|ой|ую|ые|ых|ыми)\b|"
    r"\b(?:верховн|арбитражн|конституционн)"
    r"(?:ый|ого|ому|ым|ом|ая|ой|ую|ое|ые|ых|ыми)\s+"
    r"суд(?:а|у|ом|е|ы|ов|ам|ами|ах)?\b|"
    r"\bправоприменительн(?:ый|ого|ому|ым|ом|ая|ой|ую|ое|ые|ых|ыми)\s+"
    r"практик(?:а|и|е|у|ой|ою|ам|ами|ах)?\b|"
    r"\bопределени(?:е|я|ю|ем|и|й|ям|ями|ях)\s+"
    r"(?:суд(?:а|у|ом|е|ов|ам|ами|ах)|"
    r"об\s+обеспечени(?:е|я|ю|ем|и|й|ям|ями|ях))\b|"
    r"\bдело\s+передано\s+в\s+суд\b|"
    r"\bисков(?:ый|ая|ое|ые|ого|ой|ому|ым|ую|ою|ых|ыми|ом)\s+"
    r"заявлени(?:е|я|ю|ем|и|й|ям|ями|ях)\b|"
    r"\bподать\s+жалобу(?:[.!?]\s*)?$|"
    r"\b(?:подать|подай|подайте)\s+жалобу\s+"
    r"(?:в\s+(?:суд|апелляци(?:ю|и)|кассаци(?:ю|и))|"
    r"на\s+постановлени(?:е|я|ю|ем|и|й|ям|ями|ях))\b|"
    r"\bзаявить\s+(?:несколько\s+)?исков\b|"
    r"\bвопрос\s+передан\s+суду\b"
)
_LEGAL_WEAK = (
    r"\b(?:жалоб(?:а|ы|е|у|ой|ою|ам|ами|ах)?|"
    r"определени(?:е|я|ю|ем|и|й|ям|ями|ях)|"
    r"практик(?:а|и|е|у|ой|ою|ам|ами|ах)?|"
    r"суд(?:а|у|ом|е|ы|ов|ам|ами|ах)?|"
    r"судь(?:я|и|е|ю|ей|ею|ям|ями|ях)|"
    r"иск(?:а|у|ом|е|и|ов|ам|ами|ах)?|"
    r"ист(?:ец|ца|цу|цом|це|цы|цов|цам|цами|цах)|"
    r"ответчик(?:а|у|ом|е|и|ов|ам|ами|ах)?)\b"
)
_LEGAL_NEIGHBOR = (
    r"\b(?:подан(?:а|о|ы)?|"
    r"(?:апелляционн|кассационн|надзорн|судебн)"
    r"(?:ый|ого|ому|ым|ом|ая|ой|ую|ое|ые|ых|ыми)|"
    r"вынес(?:ла|ло|ли|ен(?:а|о|ы)?)?|"
    r"округ(?:а|у|ом|е|и|ов|ам|ами|ах)?|"
    r"обжаловани(?:е|я|ю|ем|и|й|ям|ями|ях)|"
    r"инстанци(?:я|и|ю|ей|ею|ям|ями|ях)|"
    r"решени(?:е|я|ю|ем|и|й|ям|ями|ях)|"
    r"апелляци(?:я|и|ю|ей|ею|ям|ями|ях)|"
    r"кассаци(?:я|и|ю|ей|ею|ям|ями|ях))\b|"
    r"\bзаявил(?:а|и)?\s+требовани(?:е|я)\b|"
    r"\bпо\s+(?:делу|договору)\b"
)
LEGAL_HINT_RE = re.compile(
    rf"(?:{_LEGAL_STRONG})|"
    rf"(?:{_LEGAL_WEAK}).{{0,80}}(?:{_LEGAL_NEIGHBOR})|"
    rf"(?:{_LEGAL_NEIGHBOR}).{{0,80}}(?:{_LEGAL_WEAK})",
    re.I | re.S,
)
...
def selftest() -> int:
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        legal_must_stay_silent = (
            "поиск",
            "посуда",
            "рискованный",
            "определение папки",
            "результат не определен",
            "исключение при вызове Vision Framework",
            "судя по логам, песочница блокирует сервис",
            "обсудить с владельцем архитектуру",
            "рассудить, что делать дальше",
            "практически невозможно починить",
            "судьба движка неясна",
            "искать баг в коде",
            "исключить файл из сборки",
        ...
        legal_must_match = (
            ...
            "подать жалобу",
            "применить ст. 333",
            "ссылка на статью 333 ГК РФ",
            "кассационную жалобу направить в инстанцию",
            "определение об обеспечении иска",
            "постановление Пленума",
            "доводы кассационной жалобы",
            "руководствоваться постановлениями",
            "заявить несколько исков",
            "вопрос передан суду",
            "правила подсудности",
            "обзор правоприменительных практик",
            "несколько кассационных жалоб",
            "практика Верховного Суда по убыткам",
            "практика ВС РФ по убыткам",
            "решение суда первой инстанции",
            "решения Верховного Суда",
            "практика арбитражных судов",
            "позиция Верховного Суда",
            "постановлено взыскать задолженность",
            "постановлен запрет на регистрационные действия",
            "подсудимый дал показания",
            "допросить подсудимого",
            "подай жалобу в апелляцию",
        )
        assert len(legal_must_stay_silent) >= 25, "таблица бытовых оборотов ослаблена"
        assert len(legal_must_match) >= 9, "таблица правовых оборотов ослаблена"
        for text in legal_must_stay_silent:
            assert not LEGAL_HINT_RE.search(text), f"ложный правовой намек: {text!r}"
        for text in legal_must_match:
            assert LEGAL_HINT_RE.search(text), f"потерян правовой намек: {text!r}"

        t205_must_stay_silent = (
            "искал баг в коде",
            "мы искали причину",
            "определение переменной",
            "определение функции",
        ...
        t205_must_match = (
            "практика судов округа по неустойке",
            "жалоба подана в срок",
            "истец заявил требования по договору",
            "статья 333 ГК РФ применяется по заявлению суда",
            "определение от 12.03.2024 по делу N А65-1234/2023",
        ...
        assert len(t205_must_stay_silent) == len(t205_must_match) == 15
        for text in t205_must_stay_silent:
            assert not LEGAL_HINT_RE.search(text), f"ложный правовой намек: {text!r}"
        for text in t205_must_match:
            assert LEGAL_HINT_RE.search(text), f"потерян правовой намек: {text!r}"
