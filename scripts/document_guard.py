...
def check_money_propis(text: str, where: str) -> list[str]:
    ...
    for m in _MONEY_RE.finditer(text):
        ...
        if not has_cur:
            ...
            shared = words_raw and re.search(
                rf"\bи\s+(?:{_MONEY_NUM})[^.;\n]{{0,80}}"
                rf"(?:{_MONEY_CUR}){_MONEY_CUR_END}", tail, re.I)
            if not shared:
...
def selftest() -> int:
    ...
    checks = [
        ...
        ("выдача копии решения не считается общей валютой",
         check_money_propis("1 (приложение)\nпрошу выдать копию решения", "t") == []),
        ("направление копии паспорта не считается общей валютой",
         check_money_propis("1 (приложение)\nнаправляю копию паспорта", "t") == []),
        ("дата перед скобкой не считается общей валютой",
         check_money_propis("1 (приложение)\nзаключение от 26.03.2021 "
                            "(приложение 3)", "t") == []),
        ("сумма цифрами и словом рублей остается денежной находкой",
         len(check_money_propis("и 1 000 000 рублей", "t")) == 1),
