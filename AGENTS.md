...
Рабочий профиль — Codex: Astra планирует и принимает, Sol/Terra исполняют, Luna делает простую механику. Модель роли задает `model_policy.py`; составитель не принимает свой документ. Извлечение — локально, без облака и без Ollama (выведен из системы).
...
- Роли загружаются из `.codex/config.toml` и `.codex/agents/`. Если роли нет, не подменять проверку пометкой: резервный порядок — в `doc-drafter`.
- Алиасы Claude (`haiku`, `sonnet`, `opus`) в унаследованных скиллах — исторические обозначения; не передавать их в Codex. При выборе модели применять `model_policy.py` и `CODEX_ROLE_RUNTIME`.
- ПД-роль работает внутри текущего харнесса. Внешний CLI получает только обезличенный пакет через `foreign_cli.py --role second-opinion`; прямой вынос материалов дела запрещен.
- Хуки Claude в Codex не считаются исполненными. До допуска документа обязательно запустить приборы контракта, `swarm_contract.py` и `quality_gate.py`; положительный текст рецензента сам по себе не дает допуска.
- Модели одного поставщика дают независимые процессы, но не независимость поставщиков. Недоступность Claude/Kimi не скрывать и не считать отрицательным вердиктом.
- Доступ к моделям — по подпискам (решение владельца 08.09.2026). Долларовых ограничений нет; неизвестная цена не блокирует работу. Токены учитываются для экономии контекста, плановые нормы не являются квотой подписки. Реальное исчерпание квоты и отказ авторизации не скрывать; остаток подписки из числа токенов не выдумывать.
...
- Нормативный корпус подкачивает `background_corpus.py`; порядок — `docs/background-corpus.ru.md`. Не ждать очереди перед обычной работой и не запускать полную синхронную перевыгрузку из-за одного пробела. `cite.py` читает рабочую копию; для дополнительных актов есть `--resource ID`.
- Нужный дополнительный ФЗ/подзаконный акт после сверки публичных реквизитов внести через `corpus_registry.py --add` и поставить в очередь. Не сканировать все дела для пополнения. Сбой загрузки не снимает юридический допуск: непроверенную норму не выдавать за актуальную.
- Первый внешний инструмент — Линкей (`lynceuz`): `health`, затем сбор по лестнице обнаружения. ScrapeGraphAI (`sgai --json`) — только резерв после названной причины перехода. Локальная база проверяется до внешнего поиска; `health` не подменяет проверку целевого источника.
...
- Нормативная база имеет отдельный локальный граф: `python3 scripts/legal_graph.py --query 'статья 131 ГПК' --json`; статус и пересборка — `--status` / `--refresh`. Он не заменяет проверку нормы. Не смешивать его с графом кода и не сохранять в нём материалы дел.
